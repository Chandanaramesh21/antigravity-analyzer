...
class DocumentProcessor:
    ...
    def extract_image_text(self, image_bytes):
        ...
        """
        try:
            if not self.services or 'gemini_vision' not in self.services:
                raise ValueError("Gemini Vision API not initialized")
            
            # Convert image bytes if it's a file-like object
            if not isinstance(image_bytes, bytes):
                image_bytes = image_bytes.read()
                
            model = self.services['gemini_vision']
            response = model.generate_content([
                "Extract all text from this image exactly as written. If there is no text, reply with 'No text found in image'.",
